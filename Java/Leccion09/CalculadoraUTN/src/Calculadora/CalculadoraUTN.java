...
public class CalculadoraUTN {
    ...
    public static void main(String[] args) {
        ...
        List<Double> historial = new ArrayList<>(); // Creamos una lista para almacenar el historial de resultados

        while (true) { // Bucle infinito para que la calculadora siga funcionando hasta que el usuario decida salir
            System.out.println("******* Aplicacion Calculadora *******");
            mostrarMenu(); // Llamamos al método que muestra el menú de opciones

            try {
                var operacion = Integer.parseInt(entrada.nextLine()); // Leemos la opción seleccionada por el usuario
                if (operacion >= 1 && operacion <= 7) { // Si la opción es válida (entre 1 y 7)
                    ejecutarOperacion(operacion, entrada, historial); // Ejecutamos la operación seleccionada
    ...
    // Método para mostrar el historial de resultados
    private static void mostrarHistorial(List<Double> historial) {
